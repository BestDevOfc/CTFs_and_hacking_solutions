...
build_container() {
  ...
  docker build -t localhost/chall-glacierchat --platform linux/amd64 . 
}
...
run_container() {
  ...
  docker run --name chall-glacierchat --rm -p 1337:80 -it localhost/chall-glacierchat
}

kill_container() {
	docker ps --filter "name=chall-glacierchat" --format "{{.ID}}" \
		| tr '\n' ' ' \
